...
export class Incentives {
  ...
  private readonly request: (endpoint: string, options?: RequestInit) => Promise<any>
  ...
  async claimIncentives(address: string): Promise<{ txHash: string } | { alreadyClaimed: true }> {
    ...
    try {
      ...
      const claimed = await this.isClaimed(incentivesData.contract_address, incentivesData.index)
  ...
  async isClaimed(contractAddress: ETH_ADDRESS, index: number): Promise<boolean> {
    return await isClaimedFromMerkleDistributor(this.chainId, contractAddress, index, this.provider)
  }
