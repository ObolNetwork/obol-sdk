...
describe('verificationHelpers', () => {
  describe('computeDomain', () => {
    ...
    it('should use GENESIS_VALIDATOR_ROOT_HEX_STRING when override is undefined', () => {
      const domainType = fromHexString(DOMAIN_VOLUNTARY_EXIT_HEX.substring(2));
      const domain1 = computeDomain(domainType, MAINNET_CAPELLA_FORK_VERSION, undefined);
    ...
    it('should handle fork versions with or without 0x prefix', () => {
      const domainType = fromHexString(DOMAIN_VOLUNTARY_EXIT_HEX.substring(2));
      const domainWithPrefix = computeDomain(domainType, MAINNET_CAPELLA_FORK_VERSION);
      const domainWithoutPrefix = computeDomain(domainType, MAINNET_CAPELLA_FORK_VERSION.substring(2));
